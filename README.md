...
Шаги 1–4 из `ARCHITECTURE.md` §10 сделаны: ячейка выполняется в ядре, вывод приезжает
в scratch-буфер, результат-таблица листается постранично прямо из parquet.
205 тестов (91 на сайдкар, 114 на Lua).

Пока нет: стабильных id ячеек и истории прогонов (шаг 5), статуса под ячейкой
и картинок (шаг 6).
...
`:JupyterTable`, `:JupyterInterrupt`, `:JupyterRestart`, `:JupyterLog`, `:JupyterStatus`,
`:JupyterStop`.
