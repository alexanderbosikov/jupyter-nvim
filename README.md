...
**Статус под ячейкой обновляется на перерисовке.** Пометка «код правили после прогона»
пересчитывается по событиям (правка текста, переход между ячейками), а не на каждое
нажатие, поэтому иногда появляется на команду позже, чем ожидаешь.
