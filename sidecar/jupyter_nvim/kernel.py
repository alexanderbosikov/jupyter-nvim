...
Три свойства, из-за которых этот модуль написан руками, а не взят из molten:
...
4. Готовность — это ответ на shell **и** живой iopub. iopub работает через PUB/SUB, подписка встаёт
   ...
   вывода. Тот же класс аварии, что дренаж iopub у molten, только с другого конца.
"""
...
class KernelSession:
    ...
    def _ready_prober(self, stop: threading.Event) -> None:
        ...
        В отличие от `wait_for_ready` у molten, здесь (а) iopub не дренируется, (б) цикл кончается на
    ...
    def restart(self) -> dict[str, Any]:
        ...
        try:
            ...
            self._streams.clear()
            # клиент пересоздаётся целиком: у molten очередь сокета переживала рестарт
            self._open_client()
        finally:
            self._restarting = False
        for ex in aborted:
