...
-- и уезжает, когда ядро ответит. Именно этой очереди не хватало в обвязке к molten.
--
...
function M.new(opts)
    ...
    local self = setmetatable({
        ...
        env = vim.tbl_extend("force", {
            -- в терминале рендерить itables нечем (§4.5): ядро должно знать это с рождения
            NB_UTILS_ITABLES = "0",
        }, opts.env or {}),
