...
-- выводами, мы отдаём путь и строку-якорь. Наложения картинок и сбитые позиции при
-- правках не воспроизводятся ровно потому, что картинка живёт в окне вывода,
-- а не поверх кода в ноутбуке.
...
---
---В headless писать нельзя вообще: терминала нет, а stderr читает прогонщик тестов.
---Именно это ломало integration_spec — после чистки картинок поток plenary оказывался
---набит escape-кодами, и файл выходил с кодом 1 без итоговой сводки. Выглядело как
---флак прогонщика, а было нашим побочным эффектом.
